...
public class BlockListScreen extends Screen {
    ...
    public BlockListScreen(Screen parent, Consumer<ResourceLocation> callback) {
        super(Component.translatable("visual_set_edit.gui.select_block"));
        this.parent = parent;
        ...
        ForgeRegistries.BLOCKS.getKeys().stream()
                ...
                .forEach(rl -> {
                    ...
                    list.addEntry(new ScrollableSelectionList.Entry(
                            Component.literal(Component.translatable(block.getDescriptionId()).getString()
                                    + " (" + rl + ")"),
                            rl,
                            null
                    ));
